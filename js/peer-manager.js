...
class PeerManager {
    ...
    generateRoomCode(peerId) {
        ...
        // Format: XXXX-YYYY where XXXX is first 4 chars, YYYY is last 4 chars
        const cleanId = peerId.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
        const code = cleanId.substring(0, 4) + '-' + cleanId.substring(cleanId.length - 4);

        // Store full peer ID for extraction
        sessionStorage.setItem('roomCode_' + code, peerId);
    ...
    extractPeerIdFromRoomCode(roomCode) {
        ...
        const storedId = sessionStorage.getItem('roomCode_' + roomCode);
        if (storedId) {
            return storedId;
        }

        // For codes generated by others, we need the full peer ID
        // This is a limitation - we'll need to store a mapping
        // For now, prompt user to get full code
        throw new Error('Please get the full connection code from the moderator');
    }
